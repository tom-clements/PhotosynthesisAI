...
class NeuralNetworkAI(BaseAI):
    ...
    def play_turn(self, game):
        moves = (
            self.starting_moves(game.board) if game.board.round_number in [0, 1] else self.moves_available(game.board)
        )
    ...
    def play_move(self, game: Game, moves: List[Move]):
        ...
        action = np.random.choice(
            game.total_num_actions, p=self.policy(start_state, available_actions=available_actions)
        )
